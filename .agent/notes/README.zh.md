...
活跃生命周期目录树就是工作清单：浏览其生命周期/类别文件夹，或搜索仓库即可。请勿添加集中式 `INDEX.md`；不设索引的决策记录在其自己的 Agent Note 中。未来指导价值较低的已实施记录会移至下文所述、单独冻结的 [`archived/`](archived/AGENTS.md) 目录树。

<a id="classification"></a>

## 分类

每份 Agent Note 属于由笔记格式门禁强制执行的封闭集合里的一个路径编码类别；门禁拒绝其他文件夹。新增类别需要同时更新规范集合与本节，且本身也是一项 Agent Note 决策。
...
当一份 implemented Agent Note 记录的交付决策已经完整落地，且其决策依据不太可能再指导未来工作时，将其归档。如果其中的备选方案、归属边界、否定性保证、持久化语义或协议语义、安全规则，或者重新引入条件仍有价值，则继续作为活跃记录保留。绝不归档 proposed Agent Note：过时的提案应转为 rejected。仅当 rejected Agent Note 仍能避免一种可能发生的错误时保留；否则一并删除其英文、中文和伴随记录文件。请使用经过校准的归档工作流——本项目的归档技能或等效流程——不要根据字数、存续时间或目标配额来判断。
...
封存后，每组归档文件都永久冻结。禁止编辑、翻译、重新格式化、更新、移动或删除，也不得将其视为当前行为的权威依据。文档门禁会跳过归档源文件，包括其中的出站链接；当活跃文档有意引用历史时，仍可链接到归档 Agent Note。专门的归档校验器强制执行封闭的类别目录树、完整的三文件配对、归档元数据、伴随记录 hash，以及仅追加的冻结内容 manifest（元数据清单）。归档政策决策记录在其自己的 Agent Note 中。
...
每份活跃 Agent Note 遵循统一的文件内格式，由项目的笔记格式门禁（文档门禁套件的一环）强制执行；该格式的设计动机及其否决的替代方案记录在统一格式 Agent Note 中。归档记录保留封存时的格式，并增加上述归档日期行。
...
`## Decision` 以现在时态描述已交付的现实，整个文件按 [implemented/AGENTS.md](implemented/AGENTS.md) 的要求与之保持同步。`## Consequences` 记录权衡的代价**与**收益。提案阶段的标题在此属于规格用语，门禁会拒绝它们：`## Proposal`、`## Plan`、`## Migration plan` 和 `## Acceptance criteria` 不得出现在 implemented Agent Note 中（文档标准的 slop 检查清单说明了原因）。`## Testing`、`## Deferred` 或 `## Related` 章节在陈述现在时态的事实时是允许的。
...
替代方案是记录下来的，不是凭空编造的。在本格式采用之前编写、且替代方案无法从记录中重建的 Agent Note，用以下精确注释代替该章节，门禁仅对此类格式规范之前的文件接受此注释：
...
`.zh.md` 对侧文件按项目的 i18n 约定逐章节与其英文对侧文件保持相同结构；机器检查的头部标记（`# Agent Note: ` 和 `Status:` 行）保持英文原样不翻译。格式门禁跳过 `.zh.md` 文件；配对门禁检查它们的一致性。
