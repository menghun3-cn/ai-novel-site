...
These Agent Notes describe shipped decisions. Follow the [root instructions](../../../AGENTS.md) when present, the documentation standard, and the [Agent Note format](../README.md#the-file-format); the note-format gate gates the lifecycle-specific structure.
...
When a shipped note is unlikely to guide future work, archive its complete triplet through the project's archive workflow instead of continuing to maintain it.
