...
（`feat|fix|chore/<topic>`）上进行，并且只能通过 `--no-ff` 合并落地，
合并提交信息以编号 PR 的形式指名该变更（`PR #N: <topic>`）。
禁止向 `master` 或 `develop` 直接提交工作提交。分支推送到 `origin`，
同一份历史随时可以在 GitHub 补开真实 PR；由于工作环境中没有 `gh` CLI，
PR 评审环节目前由本地的「分支 + 合并提交」拓扑模拟——这足以让每个变更
独立可回滚、可追溯归属。

## Alternatives considered

**GitHub flow（所有 PR 直入 `master`）。**现阶段落选：没有 `gh` CLI，PR
环节反正要在本地模拟；而里程碑级的工作需要一个可见的集成点让 V2 各特性
先汇聚再发布合并，若每个特性都直入发布线，里程碑边界处「打不打标签」
就会变得含混。
