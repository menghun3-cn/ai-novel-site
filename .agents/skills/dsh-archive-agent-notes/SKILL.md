...
5. Run `pnpm run seal-archived`. Its append-only mode first proves every existing seal still matches, then adds only the new triplet hashes. Run the normal verifier afterward.
...
Run the archive verifier's focused test, `pnpm run verify-archived-agent-notes`, `pnpm run doc-sync`, and `git diff --check`; select any additional evidence through your repository's own pre-push check selection.
