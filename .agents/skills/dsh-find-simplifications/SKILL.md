...
This skill helps turn a broad "find things to simplify" request into evidence-backed Agent Notes that remove or collapse existing surface area. It is guidance, not a checklist: follow the code, keep judgment active, and prefer a few well-proven candidates over a pile of thin guesses.

## Start With Repo Context

- Read `AGENTS.md`, especially the pre-release stance and the conventions (including the tests-are-not-golden-truth and Agent Notes-are-not-golden-truth doctrines).
- Skim the architecture documentation before judging anything under the main packages; simplifications that fight the intended service map or event taxonomy need extra evidence.
- Use the Agent Note tree and its [rules](../../notes/README.md) to understand intentional architecture. Read the most relevant implemented examples under `implemented/` for calibration.
- Treat dual adapters and dual persistence backends as intentional by default. Do not propose deleting either twin/backend as "low effort" unless the user explicitly overrides that constraint. Removing an unused method or hook inside a protected seam can still be valid if it does not collapse the protected design.
...
- Hand-rolled code reimplements what a well-maintained external package or a Node builtin at the engine floor already provides, and the swap would delete the implementation plus its dedicated tests.
...
Introducing a dependency is a valid simplification move, not a policy exception: record a dependency policy in an Agent Note when your repository adopts one. When surveying, ask of protocol parsers, framers, retry/backoff loops, glob matchers, diff engines, and similar infrastructure: does a well-maintained npm package or a Node builtin at the repo's engine floor already do this?
...
- Production corpus: your repository's shipped source, examples, runtime scripts, and loader/config paths.
...
- The idea is correct but tiny. Add a targeted TODO/FIXME/XXX instead, using your repository's urgency semantics for those markers.
...
Diff the sibling branch against the main branch (`origin/master` or `origin/main`), not against the current PR branch, so you see its independent contribution. For each item:
...
For docs-only Agent Note work, run at least `pnpm run doc-sync` and `git diff --check`. For code comments or skill changes, also run the relevant validator when one exists. Select any other evidence from the outgoing diff; your repository's pre-push hook contributes what it is configured to run.
