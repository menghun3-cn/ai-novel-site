...
- **无封面书籍不再显示首字占位**:读者站所有封面位(首页主推、全部小说卡片、
  发现卡片、热门榜、最新更新、搜索、书籍详情)改为按 分类/题材 渲染书本形
  SVG 封面(300×400)——保留参考稿的「书」骨架(书脊/封面/页边/书签绳/投影),
  并按题材切换配色、装饰母题、字体与标题排布:治愈/科幻/仙侠/悬疑/言情/都市/
  校园/历史/冒险/默认 共 10 套;纯中文书名竖排、含拉丁/数字的横排并按词换行。
- **列表行小图标**:最新更新与热门小说排名行改用 48×48 圆角小图标
  (书脊 + 水彩色块 + 题材母题 + 题材字),不再把大封面缩成看不清的缩略图。
- **上传封面保持权威**:书有真实封面(`coverPath`)时一律显示原图,动态封面
  只做兜底;隐藏/不存在的书不生成封面(404)。
- **版本**:四包对齐 **8.3.8**;验证 `npm run test:cover`(37 项断言)全绿,
  web typecheck 通过。

### V10.7.5 修复:用户端分类浏览折叠 + 分类标题去重(v8.3.7)

- **「全部小说」页分类不再一次性全展开**:默认只展示按书籍数量排序的前
  8 个主要分类,末尾提供「展开全部/收起」按钮切换显示全部分类(约 78 个
  种子题材 + 短篇自动创建的「短篇小说」);URL 选中的分类即使不在前 8 也
  保持可见,筛选行为零变化。
- **分类详情页标题文案去重**:分类名以「小说」结尾时(如「短篇小说」)不再
  拼接出「短篇小说小说」,页面标题与浏览器标签页同步修复;普通题材分类
  (「科幻」→「科幻小说」)标题不变。

### V10.7.4 修复:kokoro 并发合成内存峰值叠加 + 长文本撞 CF 超时墙,线上听书 502/524(v8.3.6)

- **线上听书报 502/524 的根因不是 CF 拦截或 nginx 配置**:实测源站直连 GET 200
  (0.09s)、POST 20 字 200(4.6s),但 POST 200 字 180s 无响应、公网经 CF POST
  20 字 524(136.9s)。主机构 1.8G 内存被 next-server + postgres 吃满(available
  仅 15~23MB)后,kokoro 合成峰值内存全换页到 swap,20 字正常 4.6s、200 字变
  180s+,CF 免费版回源超时 ~100s 先掐断 → 524/502。nginx `proxy_read_timeout
  300s` 不是瓶颈,改它无效(CF 层先超时)。
- **修复一(合成串行化)**:`synthesizeKokoro` 改为 promise 链互斥队列——同一
  时刻只跑一个 onnxruntime CPU 推理,并发听书不再叠加内存峰值,换页风暴的
  直接推手被消除;
- **修复二(文本上限)**:kokoro 单次合成上限 300 字(超限 400 拒绝),拦截
  手误/恶意长文本;前端 TtsPlayer 实际切片约 52 字/次,完全不受影响;
- **next-server 堆保持 768MB 不降**:内存细分显示 1.05GB RSS 中 V8 堆仅
  ~157MB(受上限约束、GC 正常),大头是 onnxruntime 原生内存 + Next 运行时,
  再压有 OOM 风险且收益小。
- **部署生效**:`./update.sh` 重建容器即可(本次仅代码改动,镜像层会重建)。
