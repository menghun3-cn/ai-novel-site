...
export function buildCreationPrompt(brief: StoryBrief | undefined): string {
  ...
  const parts = [
    ...
    // 批量生成时每篇独立成文:若人物设定只给角色定位而未给具体姓名,须原创主角名,
    // 禁止套用常见默认名(如"林晓""李明""苏晚"等)或沿用同题材的惯性命名,保证多篇主角名互不相同。
    '- 主角姓名须原创:创作要求中的人物设定若未指定具体姓名,请为本故事的主角自拟一个自然、少见的中文姓名,不得使用常见模板化姓名;若已指定姓名则严格沿用。',
  ];
  return parts.join('\n');
}
