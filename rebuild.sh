...
# 用法: ./rebuild.sh                  # 默认:启用本地 Kokoro TTS + 自动下载模型(推荐)
#       ./rebuild.sh --clean          # 不用缓存,完全重建(依赖清单变更后如遇缓存异常再用)
#       ./rebuild.sh --no-tts         # 不装本地 TTS 引擎(镜像更小,听书仅 Edge 在线)
#       ./rebuild.sh --no-model       # 不自动下载模型(已有模型/手动放置时用)
#       ./rebuild.sh --clean --no-tts # 组合参数,顺序不限
set -e
cd "$(dirname "$0")"

# ── 参数解析(默认:本地 TTS + 模型下载;用 --no-* 显式关闭)────────────────────
CLEAN=0
ENABLE_TTS=1
FETCH_MODEL=1
for arg in "$@"; do
...
if [ "$FETCH_MODEL" = "1" ]; then
  MODEL_ONNX="models/kokoro/onnx/model_quantized.onnx"
  if [ -s "$MODEL_ONNX" ]; then
    # 关键权重已存在(非空)→ 整段跳过,不做任何网络请求
    echo "[rebuild] 模型已存在,跳过下载: $MODEL_ONNX"
  else
    ...
    for f in config.json tokenizer.json tokenizer_config.json onnx/model_quantized.onnx; do
      ...
      if [ ! -s "$dest" ]; then
        echo "  ⤓ $f"
        curl -fsSL --max-time 900 -o "$dest" "$BASE/$f" || { echo "  ✗ $f 下载失败(可重跑本脚本或手动放置)"; rm -f "$dest"; }
      else
        echo "  ✓ $f 已存在,跳过"
      fi
    done
